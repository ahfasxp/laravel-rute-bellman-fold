                ...
                <form action="{{ route('graphs.update', [$graph]) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="source" id="source">
                            ...
                            @foreach ($coordinates as $item)
                                <option value="{{ $item->vertex }}" {{ $graph->source == $item->vertex ? 'selected' : '' }}>{{ $item->vertex }} - {{ $item->name }}</option>
                            @endforeach
                        </select>
                        @error('source')
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="destination" id="destination">
                            <option selected="true" disabled="disabled">Pilih Vertex</option>
                            @foreach ($coordinates as $item)
                                <option value="{{ $item->vertex }}" {{ $graph->destination == $item->vertex ? 'selected' : '' }}>{{ $item->vertex }} - {{ $item->name }}</option>
                            @endforeach
                        </select>
                        @error('destination')
                            <small class="form-text text-danger">{{ $message }}</small>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="weight" id="weight" value="{{ $graph->weight }}"
